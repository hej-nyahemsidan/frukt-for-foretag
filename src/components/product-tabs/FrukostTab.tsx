...
const FrukostTab: React.FC<FrukostTabProps> = ({ selectedDays, currentDay, orderType, isPublicPage = false }) => {
  ...
  const fetchProducts = async () => {
    try {
      ...
      console.error('Error fetching frukost products:', error);
    } finally {
      setLoading(false);
    }
  };
    ...
    <>
      <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        {products.map((product) => {
          const price = product.prices['default'] || Object.values(product.prices)[0] || 0;
        ...
        {products.length === 0 && !loading && (
          <div className="col-span-full text-center py-8">
            <p className="text-gray-500">Inga produkter i denna kategori ännu.</p>
          </div>
        )}
      </div>
              ...
              <div className="space-y-3 pt-2">
                <div className="flex items-baseline gap-3">
                  <span className="text-3xl font-bold text-red-600">
                    SEK {selectedProduct?.prices['default'] || Object.values(selectedProduct?.prices || {})[0] || 0}.00
                  </span>
                </div>
              </div>
            </div>
